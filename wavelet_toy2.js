...
function Init() {
    ...
    let ImgPreview = document.getElementById("natural-preview");
    ...
    function HandleImageLoaded() {
        ...
        let ImageData;
        {
            let canvas = document.getElementById("resized-preview");
...
function CreateFlowContainer(name) {
    let ResultsContainer = document.getElementById("results");
    let NewResultContainer = document.createElement("div");
    ResultsContainer === null || ResultsContainer === void 0 ? void 0 : ResultsContainer.appendChild(NewResultContainer);
    let FlowContainer = document.createElement("div");
    FlowContainer.classList.add("flow-container");
    let NameElement = document.createElement("p");
    NameElement.appendChild(document.createTextNode(name));
    NewResultContainer === null || NewResultContainer === void 0 ? void 0 : NewResultContainer.appendChild(NameElement);
    NewResultContainer === null || NewResultContainer === void 0 ? void 0 : NewResultContainer.appendChild(FlowContainer);
    return FlowContainer;
}
function StripInsignificantCoefficients(data, num_coefficients) {
...
function TestTransform(arr, width, height) {
    let total = width * height;
    for (let i = 0; i < total; ++i) {
        arr[i] *= 0.5;
    }
}
function SetCanvasContent(canvas, data) {
    canvas.width = data.width;
    canvas.height = data.height;
    let c2d = canvas.getContext("2d");
    c2d.putImageData(data, 0, 0);
}
function AddCanvas(container, name, data) {
...
function RGB_to_norm(ImageData) {
    let orig = ImageData.data;
    let norm255 = 1.0 / 255.0;
    let NumPixels = ImageData.width * ImageData.height;
    let Channels = new Array(3);
...
function Visualize_Linear_AutoScale(d) {
    ...
    for (let c = 0; c < d.Channels.length; ++c) {
        ...
        for (let i = 0; i < NumPixels; ++i) {
            ...
            if (value_abs > MaxValues[c])
                MaxValues[c] = value_abs;
        }
    }
    for (let i = 0; i < NumPixels; ++i) {
...
function Visualize_Linear_FixedRange(d, range) {
    let NumPixels = d.Width * d.Height;
    let length = NumPixels * 4;
    let rgb = new Uint8ClampedArray(length);
    // NOTE: The initial values are deliberately 0 here:
...
function GetFileFromFileInput(e) {
    let files = e.files;
    if (files) {
        let tmp = files[0];
        return tmp !== null && tmp !== void 0 ? tmp : null;
    }
    else {
        return null;
    }
...
function reverse_2d_pass_minor(data, width, height) {
    let result = new Float64Array(data.length);
    let stride = width;
...
function transform_2d_pass_minor_weighted(data, width, height) {
    // This transform doesn't work inplace, we'd need a buffer of atleast width/2 elements.
    // Since we can't do it inplace anyway, we just allocate an entirely new result array for the lulz.
    let result = new Float64Array(data.length);
    // stride is equal to width because every array element is a pixel.
...
function reverse_2d_pass_minor_weighted(data, width, height) {
    let result = new Float64Array(data.length);
    let stride = width;
...
function GetHighestCoefficientIndices(channels, NumWantedACCoefficients) {
    // DC coefficients:
    let DCCoefficients = new Array(channels.length);
    for (let c = 0; c < channels.length; ++c) {
        DCCoefficients[c] = channels[c][0];
    }
    // AC coefficients:
    let ACCoefficients = new Array(channels.length);
    for (let c = 0; c < channels.length; ++c) {
        ACCoefficients[c] = new Array(NumWantedACCoefficients);
        let channel = channels[c];
        // Skip the first element because that already goes in the DCCoefficients.
        let bleh = new Array(channel.length - 1);
